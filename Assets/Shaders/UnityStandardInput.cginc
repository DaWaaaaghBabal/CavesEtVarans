...
{
	half ratio = tex2D(_FactionTex, texcoords.xy).a;
	half3 color = tex2D (_MainTex, texcoords.xy).rgb;
	half3 factionColor = _FactionColor.rgb * (color.r + color.g + color.b) / 3;
	half3 albedo = _Color.rgb * color * (1-ratio) + factionColor * (ratio);
#if _DETAIL
	#if (SHADER_TARGET < 30)
